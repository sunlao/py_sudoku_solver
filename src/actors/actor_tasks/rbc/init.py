...
from actors.actor_tasks.rbc.helpers.evaluate import Start
...
class Init:
    ...
    async def _send_rbc(
        ...
        await side_effects.gather(
            *(self.send.post_rbc_init(side_effects, c) for c in cells)
        )
    ...
    async def director(
        ...
        rbc_old = dto.content
        rbc_new = self.solver.all(side_effects, dto.content)
